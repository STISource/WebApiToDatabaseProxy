...
SELECT         
    ...
    AuftragPosMain.Artikel_Menge AS Quantity,
    AuftragPosMain.Artikel_Einheit AS Unit,
    CASE AuftragMain.bStatus_geliefert WHEN 1 THEN 'Geliefert' ELSE CASE WHEN QuantityAlreadyDelivered > 0 THEN 'Teilgeliefert' ELSE 'Nicht geliefert' END END AS DeliveryStatus,  
    ...
    ), 0) AS QuantityAlreadyDelivered,
    (AuftragPosMain.Artikel_Menge - QuantityAlreadyDelivered) AS QuantityOutstanding,            
...
WHERE AuftragMain.Auftragskennung = 1 
        ...
        AND DeliveryStatus != 'Nicht geliefert'
        AND DeliveryDate >= (CONVERT(DATE, GETDATE()) - (DATEPART(weekday, GETDATE()) - 2))
