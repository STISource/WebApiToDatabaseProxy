...
ORDER BY Warengruppe.Bezeichnung, Artikel.ArtikelNr";
        ...
        private const string InventoryValuationSql_v2 = @"SELECT 
    Warengruppe.Bezeichnung AS ProductGroup,
    ...
    -- lType = 3 sind manuelle Lagerbewegungen
	CASE WHEN (SELECT COUNT(*) FROM F1.FK_LagerJournal
	            where szArtikelNr = Artikel.ArtikelNr and lType = 3 and tsAktion >= DATEADD(month, -24, CURRENT_TIMESTAMP)) > 0
    THEN 'M' ELSE NULL END AS ManualMovementLast2Years	
